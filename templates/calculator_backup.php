            ...
            <div><strong>Total Project Cost:</strong> <span id="totalProjectCostDisplay" style="color: #cc0000; font-weight: bold;">$<?= isset($_GET['slab_price']) ? htmlspecialchars($_GET['slab_price']) : '1000' ?></span></div>
					...
					<!-- <div class="stats-table">
						...
						<div class="stat-column">
							...
							<div class="stat-value">$<?= isset($_GET['slab_price']) ? htmlspecialchars($_GET['slab_price']) : '1000' ?></div>
...
// 					const activeGroup = canvas.getActiveGroup();
				...
				jQuery('#toolbar .btns ul.dropdown li').click(function() {
					...
					if (downloadType === 'EMAIL') {
						...
						// Use enhanced PDF generator with admin templates
						generateEnhancedPDF(canvas, data);
					}
				...
				function calculateCosts() {
					...
					const slabCost = parseFloat('<?= isset($_GET['slab_price']) ? $_GET['slab_price'] : '1000' ?>') || 1000;
					...
					const onlyCutCost = ((onlyCutAreaMM || 0) * rateStandard) / 1000;
					const mitredCutCost = ((mitredEdgeAreaMM || 0) * rateMitred) / 1000;
					const productionCost = onlyCutCost + mitredCutCost;
					
					// Total project cost
					const totalProjectCost = (slabCost || 0) + productionCost + installationCost;
					
					// Update cost displays
					jQuery('#slabCostDisplay').html('$' + (slabCost || 0).toLocaleString());
				...
				// Function to generate enhanced HTML - REMOVED (duplicate function)
				// Using the simplified version at the end of the file instead
					...
					jQuery.ajax({
						...
						success: function(response) {
							...
							if (response.success) {
								...
								console.log('- Filename received:', response.data.filename);
								...
								if (!response.data.filename) {
									...
									alert('Enhanced PDF generation failed: No filename in response');
									return;
								}
								
								console.log('✅ Response data validation passed');
								
								// Simple PDF download - no database storage
								console.log('📥 Starting simple PDF download...');
								console.log('- Filename:', response.data.filename);
								
								// Create download URL and trigger download
								const downloadUrl = stone_slab_ajax.ajaxurl + '?action=ssc_download_pdf&pdf=' + response.data.filename + '&nonce=' + (drawingNonce || 'test');
								...
								alert('✅ PDF generated and download started! No data was saved to database.');
							} else {
				...
				// Basic PDF generation function removed - using enhanced PDF generation instead
		...
		<script>
			// Simple PDF Generator Function (No Database Storage)
			function generateEnhancedPDF(canvas, data) {
				console.log('🚀 Generating simple PDF with admin templates (no database storage)...');
				
				// Show loading message
				const loadingMsg = 'Generating PDF with admin templates...';
				alert(loadingMsg);
				
				// Prepare canvas data for PDF generation
				const canvasData = canvas.toJSON();
				const drawingData = {
					name: data.name || 'Untitled Project',
					total_cutting_mm: data.total_cutting_mm || 0,
					only_cut_mm: data.only_cut_mm || 0,
					mitred_cut_mm: data.mitred_cut_mm || 0,
					slab_cost: data.slab_cost || '0.00',
					notes: data.notes || ''
				};
				
				// Convert canvas to image for PDF
				canvas.setZoom(1); // Reset zoom for consistent image
				const canvasImage = canvas.toDataURL({
					format: 'jpeg',
					quality: 0.9
				});
				
				// Send data to server for enhanced PDF generation
				jQuery.ajax({
					url: stone_slab_ajax.ajaxurl,
					type: 'POST',
					data: {
					...
					success: function(response) {
						...
						if (response.success) {
							...
							alert('✅ PDF generated and downloaded successfully!\n\nFile: ' + response.data.filename + '\nQuote ID: ' + response.data.quote_id + '\n\nNote: This is a simple download - no data was saved to database.');
						} else {
